...
resultsFolder = '../../../Results/NILB/Perfusion NLME Spleen/';
...
%     data.spleen(idx) = []; data.spleenSE(idx) = [];
    ...
    Y = [Y; data.liver'];
    X = [X; [data.time' data.spleen' data.spleen']];
...
ve    = 0.45;                  % Extracellular volume fraction
...
% Start Guess
x0 =  [Fp,   fa,   ve,   ki,   kef];
ParamTransform = [1 1 1 1 1];

fun = @(param,simInput)perfConvSpleen(param,simInput);
